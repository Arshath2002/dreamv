 ...
 * @LastEditTime: 2025-07-21 14:22:52
-->
<template>
      ...
      <el-scrollbar class="right-scroller">
        ...
        <div class="map-relation__item" v-for="(it, index) in category" :key="index">
          <div class="map-relation__item-input">
            <div class="cell-item">
              <div class="config-title">{{ categoryName }}</div>
              <SelectInput v-model="it.valueRelation" :mapFieldList="mapFieldList" />
            </div>
          </div>
        </div>
...
export default {
  name: "DesDVMixinAxisRelation",
  mixins: [$mixins.propertyDialogSetting, chartsHelper],
  components: { SettingDataOrigin, SelectInput },
  props: ["config"],
  computed: {
    categoryName() {
      return this.config.axisType === "scatter" ? "x轴值" : "类目轴";
    },
    category() {
      if (this.config.axisType === "scatter") {
