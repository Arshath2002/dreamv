 ...
 * @LastEditTime: 2025-06-11 11:17:55
-->
<template>
  <div
  ...
  >
    ...
    <div class="back-map" @click="backMapFn" v-if="showBack" :style="backStyle">
      <template v-if="isDesign">
        <DsfIcon name="zuo" :style="{ fontSize: backTextConfig.fontSize }" />
        返回
      </template>
      <template v-else>
        <DsfIcon name="zuo" :style="{ fontSize: backTextConfig.fontSize }" />
...
<script>
import {createScatterMap} from "_dataview/output/mapDefault/scatterMap.js";
...
export default dsf.component({
  ...
  props: {
    ...
    //0：map数据， 1： 散点， 2： 动效点， 3： 正方形， 4： 立方体， 5： 圆柱
    excelData: {
      type: Array,
      default: () => {
