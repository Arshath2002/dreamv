import { ArcCurve, BufferAttribute, BufferGeometry, Color, Line, LineBasicMaterial, Points, PointsMaterial, Quaternion, Vector3 } from "three";
/*
 * 绘制一条圆弧飞线
 * 5个参数含义：( 飞线圆弧轨迹半径, 开始角度, 结束角度)
 */
function createFlyLine(radius, startAngle, endAngle, options) {
 ...
 */
export const flightyArc = (radius, sphereCoord1, sphereCoord2, options) => {
...
function arcXOY(radius, startPoint, endPoint, options) {
  ...
  const arcTopCoord = dir.multiplyScalar(radius + earthRadianAngle * radius * 0.1); // 飞行线的高度
  ...
  // 调用圆弧线模型的绘制函数
  const arcLine = circleLine(flyArcCenter.x, flyArcCenter.y, flyArcR, startAngle, endAngle, options.lineColor);
  ...
  // 绘制一段飞线，圆心做坐标原点
  const flyLine = createFlyLine(flyArcR, startAngle, startAngle + flyAngle, options);
  ...
  flyLine._speed = options.speed / 100;
...
function circleLine(x, y, r, startAngle, endAngle, color = "#d18547") {
